...
create or alter function GEN_PASSWORD (
...
declare variable PWD varchar(100);
begin
  ...
  /* if the password consists of only lowercase or only uppercase letters,
     ...
     NB! If the minimum password length is reduced to 1 character, the function
  ...
  if (PWD = upper(PWD) or PWD = lower(PWD)) then
    PWD = GEN_PASSWORD(:MIN_LENGTH, :MAX_LENGTH);
  return PWD;
end^

SET TERM ; ^

COMMENT ON FUNCTION GEN_PASSWORD IS
...
GRANT EXECUTE ON FUNCTION GEN_PASSWORD TO FUNCTION GEN_PASSWORD;